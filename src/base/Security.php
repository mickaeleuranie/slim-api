...
class Security
{
     ...
     * @throws BadRequestException if wrong length is specified
     * @throws Exception on failure.
     */
    public function generateRandomKey($length = 32)
    {
        if (!is_int($length)) {
            throw new BadRequestException('First parameter ($length) must be an integer');
        }

        if ($length < 1) {
            throw new BadRequestException('First parameter ($length) must be greater than 0');
        }
        ...
        throw new ApiException('Unable to generate a random key');
    }
